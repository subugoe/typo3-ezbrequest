...
class tx_ezbrequest_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	protected function getListView($listParams, $itemParams) {
		...
		if ($search) {
			...
			$URL = $this->conf['ezbSearchURL'] . '?' . $this->paramString($listParams, self::LINK_EZB_SEARCH_QUERY) . 'hits_per_page=100000';
		...
		if (($search) && isset($hits) && ($hits > 0)) {
			$navi = '<span class="hits">' . $hits . ' ' . \TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('hitText', 'ezbrequest') . '</span> ' . $navi;
		}
	...
	protected function getFullText($journal) {
		...
		if ($journal->detail->fulltext) {
			...
			$moreValues = $this->pi_linkToPage(substr($journal->detail->fulltext, 0, 50) . '...', $journal->detail->fulltext, '_blank', '');
	...
	protected function getZdbNumber($journal) {
		...
		if ($journal->detail->ZDB_number) {
			...
			$oldATagParams = $GLOBALS['TSFE']->ATagParams;
			$GLOBALS['TSFE']->ATagParams = ' class="external-link-new-window" ';
			$zdbNumber = $this->pi_linkToPage($journal->detail->ZDB_number, $journal->detail->ZDB_number['url'], '_blank', '');
